...
if python -c "import gsplat" 2>/dev/null; then
    ...
    [ "$VRAM_GB" -le 4 ] && MAX_JOBS=1

    info "RAM : ${RAM_GB}GB | MAX_JOBS auto : $MAX_JOBS"
    info "Compilation gsplat (~10-20 min)..."
    MAX_JOBS=$MAX_JOBS pip install gsplat --no-build-isolation -q
    ok "gsplat installe"
fi
